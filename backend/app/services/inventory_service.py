...
def create_inventory_item(
    ...
    try:
        ...
        err_msg = str(e).lower()
        is_missing_column = (
            "has no column named" in err_msg
            or "no such column" in err_msg
            or "column does not exist" in err_msg
        ...
        if is_missing_column:
            ...
            for col, col_type in COLUMN_TYPES.items():
                try:
                    db.execute(text(f"ALTER TABLE inventory_items ADD COLUMN {col} {col_type}"))
                    db.commit()
                    ...
                    try:
                        ...
                        pass
                    alter_err = str(alter_exc).lower()
                    if "duplicate" in alter_err or "already exists" in alter_err:
                        continue
                    logger.exception("Schema migration failed adding column '%s' (%s) to inventory_items: %s", col, col_type, alter_exc)
                    raise HTTPException(
                        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                        detail=f"Schema migration failed while adding column '{col}'. Error: {alter_exc}",
                    ) from alter_exc
            ...
            try:
                ...
                logger.exception("Failed to insert inventory item after schema migration: %s", commit_exc)
                try:
                    db.rollback()
                except Exception:
                    pass
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Database error creating inventory item after schema migration.",
                ) from commit_exc
        else:
            raise e
    db.refresh(item)
