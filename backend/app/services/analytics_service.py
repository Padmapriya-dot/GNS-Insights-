...
def get_worker_performance_score(db: Session, tenant_id: int) -> dict:
    ...
    try:
        ...
        logger.exception("get_worker_performance_score database error for tenant %s: %s", tenant_id, exc)
        db.rollback()
        raise
    except Exception as exc:
        logger.exception("get_worker_performance_score unexpected error for tenant %s: %s", tenant_id, exc)
        raise
