...
import { DEMO_CUSTOMERS, enrichApiCustomer } from "../../data/customersMasterData";
...
function DeleteConfirmModal({ open, onClose, onConfirm }) {
  ...
  return createPortal(
    ...
    >
      <div className="w-full max-w-[470px] rounded-2xl bg-white p-8 text-center shadow-2xl">
        <div className="mx-auto mb-6 grid h-16 w-16 place-items-center rounded-full bg-[#fee2e2] text-[#ef4444]">
          <Trash2 className="h-8 w-8" />
        </div>
        <h3 className="text-[44px] font-bold text-[#3a3a42]">Delete Customer?</h3>
        <p className="mt-3 text-[17px] text-[#4a4a55]">
        ...
        <div className="mt-7 grid grid-cols-2 gap-5">
          <button
            type="button"
            onClick={onClose}
            className="rounded-xl bg-[#eceef4] py-3 text-[34px] font-semibold text-[#3a3a42]"
          >
            No
          </button>
          <button
            type="button"
            onClick={onConfirm}
            className="rounded-xl bg-[#ef5350] py-3 text-[34px] font-semibold text-white"
          >
            Delete
          </button>
        </div>
...
export default function Customers() {
  ...
  const rows = filteredCustomers.slice((page - 1) * pageSize, page * pageSize);
        ...
        <div className="rounded-xl border border-[#d0d0d8] bg-white p-4 shadow-sm">
          ...
          <div className="overflow-hidden rounded-lg border border-[#ececf0]">
            <table className="w-full border-collapse text-left text-[12px]">
              ...
              <tbody>
                {rows.map((c) => (
                  <tr key={c.id} className="border-b border-[#f0f0f4] text-[#1a1a1f]">
                    <td className="px-3 py-3">{c.company || c.name || "-"}</td>
                    <td className="px-3 py-3">{c.gstin || "-"}</td>
                    <td className="px-3 py-3">{c.email || "-"}</td>
                    <td className="px-3 py-3">{c.phone || "-"}</td>
                    <td className="px-3 py-3">{c.address_line1 || c.billing_address || "-"}</td>
                    <td className="px-3 py-3">{c.city || "-"}</td>
                    <td className="px-3 py-3">{c.state || "-"}</td>
                    <td className="px-3 py-3">{c.pincode || "-"}</td>
            ...
            {rows.length === 0 ? (
              <div className="px-3 py-10 text-center text-[13px] text-[#8a8a96]">No customers found.</div>
            ) : null}
          </div>
