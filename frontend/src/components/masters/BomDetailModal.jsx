...
export default function BomDetailModal({ bom, onClose, onEdit, onCopy, onDelete, onPrint }) {
  ...
  const [components, setComponents] = useState(bom?.components || []);
  ...
  const handleAddComponent = () => {
    ...
    if (!name?.trim()) return;
    const qty = Number(window.prompt("Quantity", "1") || 1) || 1;
    const unitCost = Number(window.prompt("Unit cost (₹)", "0") || 0) || 0;
    const row = {
      id: `comp-${Date.now()}`,
      component: name.trim(),
      item_code: `RM-${String(components.length + 1).padStart(3, "0")}`,
    ...
    setComponents((prev) => [...prev, row]);
    setSelectedComponentId(row.id);
    addToast(`Added component "${row.component}"`);
  };

  const handleRemoveComponent = () => {
    if (!selectedComponentId) {
      addToast("Select a component row first", "info");
      return;
    }
    const target = components.find((c) => c.id === selectedComponentId);
    if (!window.confirm(`Remove "${target?.component || "component"}"?`)) return;
  ...
  const handleDuplicateComponent = () => {
    ...
    const source = components.find((c) => c.id === selectedComponentId);
    if (!source) return;
    const copy = {
      ...source,
      id: `comp-${Date.now()}`,
      component: `${source.component} (Copy)`,
      item_code: `${source.item_code || "RM"}-C`,
    };
    setComponents((prev) => [...prev, copy]);
    setSelectedComponentId(copy.id);
    addToast(`Duplicated "${source.component}"`);
  };

  const handleMaterialRequirement = () => {
    const qtyPrompt = window.prompt("Finished quantity to produce", "1");
    if (qtyPrompt == null) return;
    const finishedQty = Number(qtyPrompt) || 1;
    const lines = components.map((c) => {
      const req = (Number(c.qty) || 0) * finishedQty;
      return `${c.component}: ${req} ${c.unit || ""}`.trim();
    });
    const summary = lines.length
      ? `Material for ${finishedQty} × ${bom.product_name}: ${lines.join("; ")}`
      : "No components to calculate material requirement.";
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
      <div className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
          ...
          {tab === "components" && (
            <div className="space-y-3">
              <div className="flex gap-2">
                <button type="button" onClick={handleAddComponent} className="inline-flex items-center gap-1 rounded-lg bg-[#2563EB] px-3 py-1.5 text-xs font-semibold text-white"><Plus className="h-3.5 w-3.5" /> Add Component</button>
                <button type="button" onClick={handleRemoveComponent} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600">Remove Component</button>
                <button type="button" onClick={handleDuplicateComponent} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600">Duplicate Component</button>
              </div>
              <div className="overflow-x-auto rounded-xl border border-slate-200">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody>
                    {components.length === 0 ? (
                      <tr><td colSpan={7} className="px-3 py-6 text-center text-slate-400">No components — click Add Component</td></tr>
                    ) : (
                      components.map((c) => (
                        <tr
                          key={c.id}
                          onClick={() => setSelectedComponentId(c.id)}
                          className={`cursor-pointer border-t border-slate-100 ${selectedComponentId === c.id ? "bg-blue-50" : "hover:bg-slate-50"}`}
                        >
        ...
        <div className="flex flex-wrap gap-2 border-t border-slate-100 bg-slate-50 px-5 py-3">
          ...
          <button type="button" onClick={handleMaterialRequirement} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
