...
function KpiCard({ label, value, icon: Icon, color }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-3.5 shadow-xs min-h-[86px] flex flex-col justify-between min-w-0 overflow-hidden" title={typeof label === "string" ? label : undefined}>
      ...
      <div className="mt-2">
        <p className="truncate text-xl font-extrabold tabular-nums text-slate-900 leading-none">{value ?? 0}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="min-h-full pb-8 print:p-0" style={{ background: "#F5F5F5" }}>
      <div className="mx-auto max-w-[1400px] space-y-5 px-4 py-5 sm:px-6 lg:px-8">
        <div>
          <h1 className="text-[22px] font-semibold tracking-tight text-[#1a1a1f]">Raw Materials</h1>
          <p className="mt-0.5 text-xs text-slate-500 print:hidden">
            Monitor raw material stock levels to prevent line stoppages.
          </p>
        </div>
        ...
        <div className="mb-0 flex flex-wrap items-center justify-between gap-2 print:hidden">
          <div className="flex flex-wrap gap-2">
            <Link to="/inventory/items/create?type=raw_material" className="ui-btn-secondary text-sm">
              <Plus className="inline h-4 w-4" /> New Material
            </Link>
          </div>
          <div className="flex flex-wrap gap-2">
            <button type="button" onClick={() => exportToExcel(filtered, columns.filter((c) => !c.render), "raw-materials")} className="inline-flex items-center gap-1.5 rounded-lg border border-[#e4e4ea] bg-[#f3f3f6] px-3.5 py-2 text-[13px] font-semibold text-[#1a1a1f] hover:bg-[#ececf0]">
