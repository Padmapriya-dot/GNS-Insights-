...
import { getVendors, createVendor, createPurchaseOrder } from "../../api/procurementApi";
...
export default function CreatePurchaseOrder() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(Array.isArray(data) ? data : data.items || data.inventory_items || []);
    } catch {
      addToast("Could not load vendors or inventory items.", "error");
  ...
  const chooseItem = (item) => {
    ...
    if (!id) return;
    if (lines.some((line) => String(line.item_id) === String(id))) { setModal(null); return; }
    setLines((old) => [...old, { item_id: id, name: item.name || item.item_name || item.product_name || `Item #${id}`, quantity: 1, unit_price: item.unit_price ?? item.purchase_price ?? item.price ?? "", unit: item.unit || item.unit_of_measure || "Nos" }]);
    setModal(null);
  };
  ...
  const saveVendor = async () => {
    ...
    } catch (err) { addToast(err.response?.data?.detail || "Could not add vendor.", "error"); }
  };
  const addLocalProduct = () => {
    if (!newProduct.name.trim()) { addToast("Enter a product name.", "error"); return; }
    setLines((old) => [...old, { ...newProduct, local: true, quantity: Number(newProduct.quantity) || 1 }]);
  ...
  const submit = async (event) => {
    ...
    } catch (err) { addToast(err.response?.data?.detail || "Could not create purchase order.", "error"); } finally { setSaving(false); }
  };
  ...
  return <form onSubmit={submit} className="flex h-full min-h-0 flex-col bg-[#F5F5F5] text-slate-800">
    <header className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-[#e4e4ea] bg-white px-4 py-3 md:px-6">
      ...
      <div className="flex gap-2">
        ...
        <button type="submit" disabled={saving} className={darkButton}>{saving ? "Submitting…" : "Submit"}</button>
      </div>
    </header>
      ...
      <Section
        ...
        actions={
          <>
            ...
            <button type="button" className={`${softButton} !py-1.5 !text-[12px]`} onClick={() => setModal("addProduct")}>Add New Item</button>
          </>
        }
      >
        {lines.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <OutlinedField label="Select State">
            <select
              ...
              onChange={(e) => setAddConsignee({ ...addConsignee, state: e.target.value })}
            >
      ...
      <Modal title="Add New Product" onClose={() => setModal(null)}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button type="button" className={`${outlineButton} w-full`} onClick={() => setModal(null)}>
              Cancel
            </button>
