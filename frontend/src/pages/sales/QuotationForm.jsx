...
export default function QuotationForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast("Quotation created");
      navigate("/sales/quotations");
    } catch (err) {
      console.error(err);
      addToast(err.response?.data?.detail || "Failed to save quotation", "error");
    } finally {
      setSaving(false);
    }
  };
    ...
    >
      ...
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[#e4e4ea] bg-white px-5 py-3.5">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-[20px] font-bold text-[#1a1a1f]">Create Quotation</h1>
