...
import { createPayment, getInvoicesV2 } from "../../api/salesApi";
...
export default function PaymentReceiptForm() {
  ...
  useEffect(() => {
    Promise.allSettled([
      fetchCustomersWithFallback(),
      getInvoicesV2({ page: 1, page_size: 200, payment_filter: "unpaid" }),
    ])
      .then(([custRes, invRes]) => {
        ...
        if (pre) {
          const inv = (Array.isArray(items) ? items : []).find(
            (i) => String(i.id) === String(pre)
          );
          if (inv) {
            setForm((f) => ({
              ...f,
            ...
            setSettle({ [inv.id]: Number(inv.amount_due || inv.pending_amount || inv.grand_total) || 0 });
          }
        }
      })
  ...
  const buyerInvoices = useMemo(() => {
    ...
    return invoices.filter(
      (i) =>
        String(i.customer_id) === String(form.customer_id) &&
        String(i.payment_status || i.status || "").toLowerCase() !== "paid"
    );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const receiptNumber = [form.prefix, form.receipt_number].filter(Boolean).join("") || `RCPT-${Date.now().toString().slice(-6)}`;
      const methodMap = {
        cash: "cash",
        cheque: "cheque",
        net_banking: "neft",
      ...
      if (settleRows.length > 0) {
        ...
        await createPayment({
          ...
          payment_date: form.payment_date,
          method,
          notes: JSON.stringify({ ...metaBase, is_advance: true, unused_amount: collected }),
        });
      } else {
      ...
      addToast(err.response?.data?.detail || "Failed to save payment", "error");
    } finally {
      setSaving(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="flex h-full min-h-0 flex-col bg-[#F5F5F5]">
      <div className="flex shrink-0 items-center justify-between gap-3 border-b border-[#e4e4ea] bg-white px-5 py-3.5">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-[20px] font-bold text-[#1a1a1f]">Create Payment Receipt</h1>
                          ...
                          <div className="relative">
                            ...
                            {partyMenuId === c.id ? (
                              <div className="absolute right-0 z-40 mt-1 w-44 overflow-hidden rounded-xl border border-[#ececf0] bg-white py-1 shadow-lg">
                                ...
                                <button
                                  type="button"
                                  className="flex w-full items-center gap-2 px-3 py-2 text-left text-[13px] text-[#b45309]"
                                  onClick={(e) => e.stopPropagation()}
                                >
                                ...
                                <button
                                  type="button"
                                  className="flex w-full items-center gap-2 px-3 py-2 text-left text-[13px] text-[#dc2626]"
                                  onClick={(e) => e.stopPropagation()}
                                >
