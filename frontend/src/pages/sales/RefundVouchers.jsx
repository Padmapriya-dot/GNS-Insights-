...
const YELLOW = "#F5C518";
const STORAGE_KEY = "gns_refund_vouchers";
...
export default function RefundVouchers() {
  ...
  const [rows, setRows] = useState(loadVouchers);
  ...
  const handleSave = (voucher) => {
    setRows((prev) => {
      const next = [voucher, ...prev];
      saveVouchers(next);
      return next;
    });
    // reduce unused for party in local view
    setUnusedByParty((m) => ({
      ...m,
      [String(voucher.party_id)]: Math.max(
        0,
        (Number(m[String(voucher.party_id)]) || 0) - Number(voucher.amount)
      ),
    }));
    addToast("Refund voucher created");
  };
